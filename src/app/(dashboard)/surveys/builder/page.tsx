      ...
      <div className="flex flex-col h-full bg-white">
        ...
        <div className="flex items-center justify-between px-6 py-3 border-b border-gray-200">
          <div className="flex items-center gap-3">
            <button onClick={() => router.push("/surveys")} className="text-gray-400 hover:text-gray-600">
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      selectedIdx === i ? "border-indigo-300 bg-indigo-50" : "border-gray-200 hover:border-gray-300"
                    )}
                  >
                    {/* Move buttons */}
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs text-gray-400">Q{i + 1} - {typeInfo?.label ?? q.type}</p>
                      <p className="text-sm text-gray-900 truncate">{q.text || "Untitled question"}</p>
                    </div>
                  </div>
                );
              })}
            ...
            <div className="p-4 border-t border-gray-200">
              {showTypePicker ? (
                <div className="space-y-1">
                  <p className="text-xs font-medium text-gray-500 mb-2">Select question type</p>
                  ...
                  <button onClick={() => setShowTypePicker(false)} className="w-full text-xs text-gray-400 hover:text-gray-600 mt-1 py-1">Cancel</button>
                </div>
              ) : (
                <button onClick={() => setShowTypePicker(true)}
                  className="w-full flex items-center justify-center gap-1.5 px-4 py-2 text-sm font-medium text-indigo-600 bg-indigo-50 rounded-lg hover:bg-indigo-100 transition-colors">
            ...
            {selected && (
              <div className="max-w-xl">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-sm font-semibold text-gray-900">Question {selectedIdx + 1}</h3>
                  {questions.length > 1 && (
                      ...
                      {([5, 10] as const).map((s) => (
                        <button key={s} onClick={() => updateQuestion(selectedIdx, { scale: s })}
                          className={cn("px-4 py-2 text-sm rounded-lg border transition-colors",
                            selected.scale === s ? "border-indigo-300 bg-indigo-50 text-indigo-700" : "border-gray-200 text-gray-600 hover:bg-gray-50"
                          )}>
                          1-{s}
                        </button>
                      ))}
                ...
                {selected.type === "NPS" && (
                  <div className="mb-4">
                    <label className="block text-xs font-medium text-gray-500 mb-1">Follow-up Question</label>
                    <input
                  ...
                  <div className="flex items-center gap-2 mt-4">
                    <button
                      ...
                      className={cn("w-9 h-5 rounded-full transition-colors relative",
                        selected.required ? "bg-indigo-600" : "bg-gray-300"
                      )}>
                ...
                <div className="mt-8 p-4 bg-gray-50 rounded-lg border border-gray-200">
                  ...
                  {selected.type === "SHORT_TEXT" && (
                    <input disabled placeholder={selected.placeholder || "Short answer"} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white" />
                  )}
                  {selected.type === "LONG_TEXT" && (
                    <textarea disabled placeholder={selected.placeholder || "Long answer"} rows={3} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white resize-none" />
                  )}
                  ...
                  {selected.type === "DROPDOWN" && (
                    <select disabled className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white text-gray-400">
                  ...
                  {selected.type === "RATING" && (
                    <div className="flex gap-1">
                      {Array.from({ length: selected.scale ?? 5 }, (_, i) => (
                        <span key={i} className="w-8 h-8 border border-gray-300 rounded-lg flex items-center justify-center text-xs text-gray-400">{i + 1}</span>
                      ))}
                    </div>
                  )}
                  {selected.type === "NPS" && (
                    <div>
                      <div className="flex gap-1 mb-2">
                        {Array.from({ length: 11 }, (_, i) => (
                          <span key={i} className="w-7 h-7 border border-gray-300 rounded flex items-center justify-center text-[10px] text-gray-400">{i}</span>
                        ))}
                      </div>
                      ...
                      {selected.followUp && (
                        <div className="mt-3">
                          <p className="text-xs text-gray-500 mb-1">{selected.followUp}</p>
                          <textarea disabled rows={2} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white resize-none" />
                        </div>
                      )}
                    </div>
                  )}
