...
function SkeletonCard() {
  return (
    <div className="animate-pulse bg-[var(--bg-base)] border border-[var(--border)] rounded-lg p-4">
      <div className="h-4 bg-[var(--bg-surface)] rounded w-1/3 mb-3" />
      <div className="h-3 bg-[var(--bg-surface)] rounded w-2/3 mb-2" />
    ...
    <div className="p-6 h-full flex flex-col bg-[var(--bg-base)]">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-[var(--text-secondary)] mt-0.5">Brand monitoring and sentiment tracking</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => openChatWithContext("Set up a brand monitor for my company to track mentions and sentiment.")}
      ...
      <div className="flex-1 overflow-y-auto space-y-6">
        {/* Set up a monitor */}
        <section>
          <h2 className="text-sm font-semibold text-[var(--text-secondary)] mb-3 uppercase tracking-wide">Set Up a Monitor</h2>
          <form onSubmit={handleSetupMonitor} className="flex gap-2">
            <input
              ...
              className="flex-1 border border-[var(--border-strong)] rounded-lg px-3 py-2 text-sm text-[var(--text-primary)] placeholder-[var(--text-tertiary)] focus:outline-none focus:ring-2 focus:ring-[var(--accent)]"
            />
            <button
              type="submit"
              disabled={!keyword.trim()}
              className="px-4 py-2 text-sm font-medium text-[var(--primary-foreground)] bg-[var(--accent)] rounded-lg hover:bg-[var(--accent-hover)] transition-colors disabled:opacity-50"
            >
              Track
            </button>
          </form>
        ...
        <section>
          ...
          {loading ? (
            ...
            <div className="text-center py-12 border border-[var(--border)] rounded-lg">
              <div className="w-12 h-12 bg-[var(--bg-surface)] rounded-full flex items-center justify-center mx-auto mb-3">
              ...
              <p className="text-sm text-[var(--text-secondary)] mb-1">No brand mentions detected yet</p>
            ...
            <div className="space-y-3">
              {entityEntries.map((entry) => (
                <div key={entry.id} className="bg-[var(--bg-base)] border border-[var(--border)] rounded-lg p-4 hover:border-[var(--border-strong)] transition-colors">
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-[var(--accent-subtle)] text-[var(--accent)]">
                          {entry.entryType}
                        </span>
                        <span className="text-xs text-[var(--text-tertiary)]">{entry.category}</span>
                      </div>
                      <p className="text-sm font-medium text-[var(--text-primary)]">{entry.key}</p>
                      <p className="text-sm text-[var(--text-secondary)] mt-0.5 line-clamp-2">{entry.value}</p>
                      {entry.confidence != null && (
                        <div className="flex items-center gap-2 mt-2">
                          ...
                          <span className="text-xs text-[var(--text-tertiary)]">{Math.round(entry.confidence * 100)}% confidence</span>
                        </div>
                      )}
                    </div>
                    <span className="text-xs text-[var(--text-tertiary)] flex-shrink-0 ml-4">
        ...
        <section>
          <h2 className="text-sm font-semibold text-[var(--text-secondary)] mb-3 uppercase tracking-wide">Sentiment Analysis</h2>
          <div className="bg-[var(--bg-base)] border border-[var(--border)] rounded-lg p-6">
            <div className="flex items-center gap-8">
              ...
              <div className="flex-shrink-0 relative">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <span className="text-xs font-medium text-[var(--text-tertiary)] bg-[var(--bg-base)] rounded-full px-1">sample</span>
                </div>
              </div>
              <div className="flex-1">
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm text-[var(--text-secondary)] flex-1">Positive</span>
                    <span className="text-sm text-[var(--text-tertiary)]">&mdash;</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="w-3 h-3 rounded-full bg-yellow-300" />
                    <span className="text-sm text-[var(--text-secondary)] flex-1">Neutral</span>
                    <span className="text-sm text-[var(--text-tertiary)]">&mdash;</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="w-3 h-3 rounded-full bg-red-300" />
                    <span className="text-sm text-[var(--text-secondary)] flex-1">Negative</span>
                    <span className="text-sm text-[var(--text-tertiary)]">&mdash;</span>
                  </div>
                </div>
              </div>
