      ...
      <div className="p-6 bg-[var(--bg-base)] min-h-full">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-[var(--text-secondary)] mt-0.5">Posts scheduled for future publishing.</p>
          </div>
        </div>
        ...
        {!loading && posts.length === 0 && (
          <div className="text-center py-16 border border-[var(--border)] rounded-xl">
            ...
            <p className="text-sm text-[var(--text-secondary)] mb-1">No scheduled posts</p>
            <p className="text-xs text-[var(--text-tertiary)]">Schedule drafts to see them here.</p>
          </div>
        )}
                      ...
                      {rescheduleId === p.key && (
                        <div className="mt-3 pt-3 border-t border-[var(--border)] flex items-end gap-3">
                          <div>
                            ...
                            <input type="date" value={reschedDate} onChange={(e) => setReschedDate(e.target.value)} className="border border-[var(--border-strong)] rounded-lg px-3 py-1.5 text-sm" />
                          </div>
                          <div>
                            <label className="text-xs text-[var(--text-secondary)] block mb-1">Time</label>
                            <input type="time" value={reschedTime} onChange={(e) => setReschedTime(e.target.value)} className="border border-[var(--border-strong)] rounded-lg px-3 py-1.5 text-sm" />
                          </div>
                          <button onClick={() => setRescheduleId(null)} className="px-3 py-1.5 text-xs text-[var(--text-secondary)] hover:text-[var(--text-primary)]">Cancel</button>
