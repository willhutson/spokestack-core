  ...
  return (
    <ModuleLayoutShell moduleType="FINANCE">
      <div className="p-6">
        ...
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-[var(--text-primary)]">Receivables</h1>
          <p className="text-sm text-[var(--text-secondary)] mt-0.5">Outstanding invoices and aging analysis.</p>
        </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6 animate-pulse">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="bg-[var(--bg-base)] border border-[var(--border)] rounded-xl p-5">
                <div className="h-3 w-16 bg-[var(--bg-surface)] rounded mb-2" />
                <div className="h-6 w-24 bg-[var(--bg-surface)] rounded" />
              </div>
            ))}
          </div>
        ) : (
            ...
            {bucketTotals.map((bt) => (
              <button
                ...
                className={`text-left border rounded-xl p-5 transition-all ${
                  activeBucket === bt.key
                    ? `${bucketColor(bt.key)} ring-2 ring-offset-1 ring-[var(--accent)]`
                    : `${bucketColor(bt.key)} hover:shadow-sm`
                }`}
              >
                <p className="text-xs font-medium text-[var(--text-secondary)] mb-1">{bt.label}</p>
                <p className={`text-lg font-bold ${bucketValueColor(bt.key)}`}>
                  {aedFormatter.format(bt.total / 100)}
                </p>
                <p className="text-xs text-[var(--text-secondary)] mt-1">{bt.count} invoice{bt.count !== 1 ? "s" : ""}</p>
              </button>
            ))}
          </div>
        )}
        ...
        {loading ? (
          ...
          <div className="bg-[var(--bg-base)] border border-[var(--border)] rounded-xl p-12 text-center">
            <h3 className="text-sm font-medium text-[var(--text-primary)] mb-1">No outstanding receivables</h3>
            <p className="text-xs text-[var(--text-secondary)]">All invoices are up to date.</p>
          </div>
        ) : (
          <div className="bg-[var(--bg-base)] border border-[var(--border)] rounded-xl overflow-hidden">
            <table className="w-full">
              <thead>
                <tr className="border-b border-[var(--border)] bg-[var(--bg-base)]">
                  ...
                  <th className="text-left text-xs font-medium text-[var(--text-secondary)] uppercase tracking-wider px-5 py-3">Invoice #</th>
                  <th className="text-left text-xs font-medium text-[var(--text-secondary)] uppercase tracking-wider px-5 py-3">Issue Date</th>
                  <th className="text-left text-xs font-medium text-[var(--text-secondary)] uppercase tracking-wider px-5 py-3">Due Date</th>
                  <th className="text-right text-xs font-medium text-[var(--text-secondary)] uppercase tracking-wider px-5 py-3">Amount</th>
                  <th className="text-right text-xs font-medium text-[var(--text-secondary)] uppercase tracking-wider px-5 py-3">Days Overdue</th>
              ...
              <tbody className="divide-y divide-[var(--border)]">
                {filtered.map((r) => (
                  <tr key={r.id} className="hover:bg-[var(--bg-hover)] transition-colors">
                    <td className="px-5 py-4 text-sm text-[var(--text-primary)]">{r.client?.name || "-"}</td>
                    <td className="px-5 py-4 text-sm font-medium text-[var(--text-primary)]">{r.number}</td>
                    ...
                    <td className={`px-5 py-4 text-right text-sm font-medium ${r.daysOverdue > 30 ? "text-red-600" : "text-[var(--text-secondary)]"}`}>
