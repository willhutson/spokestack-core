      ...
      <div className="p-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-[var(--text-secondary)] mt-0.5">Track budget allocation and utilization by project.</p>
          </div>
          <button
        ...
        {showForm && (
          <div className="bg-[var(--bg-base)] border border-[var(--border)] rounded-xl p-5 mb-6">
            <h3 className="text-sm font-semibold text-[var(--text-primary)] mb-4">Create Budget</h3>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full border border-[var(--border-strong)] rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-[var(--accent)] focus:border-[var(--accent)]"
                  placeholder="e.g., Website Redesign"
                />
              </div>
              <div>
                <label className="block text-xs font-medium text-[var(--text-secondary)] mb-1">Allocated (AED)</label>
                <input
                  type="number"
                  value={formAllocated}
              ...
              <div>
                <label className="block text-xs font-medium text-[var(--text-secondary)] mb-1">Spent (AED)</label>
                <input
                  type="number"
                  value={formSpent}
        ...
        {loading ? (
          ...
          <div className="bg-[var(--bg-base)] border border-[var(--border)] rounded-xl p-12 text-center">
            <h3 className="text-sm font-medium text-[var(--text-primary)] mb-1">No budgets yet</h3>
            <p className="text-xs text-[var(--text-secondary)]">Click &quot;New Budget&quot; to create your first budget entry.</p>
          </div>
        ) : (
          <div className="bg-[var(--bg-base)] border border-[var(--border)] rounded-xl overflow-hidden">
            <table className="w-full">
              <thead>
                <tr className="border-b border-[var(--border)] bg-[var(--bg-base)]">
                  ...
                  <th className="text-right text-xs font-medium text-[var(--text-secondary)] uppercase tracking-wider px-5 py-3">Allocated</th>
                  <th className="text-right text-xs font-medium text-[var(--text-secondary)] uppercase tracking-wider px-5 py-3">Spent</th>
                  <th className="text-right text-xs font-medium text-[var(--text-secondary)] uppercase tracking-wider px-5 py-3">Remaining</th>
                  <th className="text-left text-xs font-medium text-[var(--text-secondary)] uppercase tracking-wider px-5 py-3 w-48">Utilization</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[var(--border)]">
                {budgets.map((b) => {
                  ...
                  return (
                    <tr key={b.id} className="hover:bg-[var(--bg-hover)] transition-colors">
                      <td className="px-5 py-4 text-sm font-medium text-[var(--text-primary)]">{b.value.projectName || b.key}</td>
                      <td className="px-5 py-4 text-right text-sm text-[var(--text-secondary)]">{aedFormatter.format(allocated / 100)}</td>
                      <td className="px-5 py-4 text-right text-sm text-[var(--text-secondary)]">{aedFormatter.format(spent / 100)}</td>
                      <td className={`px-5 py-4 text-right text-sm font-medium ${remaining < 0 ? "text-red-600" : "text-[var(--text-primary)]"}`}>
                ...
                {/* Summary row */}
                <tr className="bg-[var(--bg-base)] border-t-2 border-[var(--border)] font-semibold">
                  <td className="px-5 py-4 text-sm text-[var(--text-primary)]">Total</td>
                  <td className="px-5 py-4 text-right text-sm text-[var(--text-primary)]">{aedFormatter.format(totalAllocated / 100)}</td>
                  <td className="px-5 py-4 text-right text-sm text-[var(--text-primary)]">{aedFormatter.format(totalSpent / 100)}</td>
                  <td className={`px-5 py-4 text-right text-sm ${totalRemaining < 0 ? "text-red-600" : "text-[var(--text-primary)]"}`}>
