      ...
      <div className="p-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-[var(--text-secondary)] mt-0.5">Monthly income and expenses overview.</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
            <div className="bg-[var(--bg-base)] border border-[var(--border)] rounded-xl p-5">
              <p className="text-xs font-medium text-[var(--text-secondary)] mb-1">Total Income</p>
              <p className="text-xl font-bold text-emerald-600">{aedFormatter.format(totalIncome / 100)}</p>
            </div>
            <div className="bg-[var(--bg-base)] border border-[var(--border)] rounded-xl p-5">
              <p className="text-xs font-medium text-[var(--text-secondary)] mb-1">Total Expenses</p>
              <p className="text-xl font-bold text-red-600">{aedFormatter.format(totalExpenses / 100)}</p>
            </div>
            <div className="bg-[var(--bg-base)] border border-[var(--border)] rounded-xl p-5">
              <p className="text-xs font-medium text-[var(--text-secondary)] mb-1">Net Cash Flow</p>
        ...
        {!loading && (
          <div className="bg-[var(--bg-base)] border border-[var(--border)] rounded-xl overflow-hidden">
            <div className="px-5 py-3 border-b border-[var(--border)] bg-[var(--bg-base)]">
              <h3 className="text-sm font-semibold text-[var(--text-primary)]">Monthly Breakdown</h3>
            </div>
            {data.length === 0 ? (
              <div className="p-12 text-center">
                <h3 className="text-sm font-medium text-[var(--text-primary)] mb-1">No cash flow data</h3>
                <p className="text-xs text-[var(--text-secondary)]">Data will appear once orders are recorded.</p>
              </div>
            ) : (
              <table className="w-full">
                <thead>
                  <tr className="border-b border-[var(--border)] bg-[var(--bg-base)]">
                    <th className="text-left text-xs font-medium text-[var(--text-secondary)] uppercase tracking-wider px-5 py-3">Month</th>
                    <th className="text-right text-xs font-medium text-[var(--text-secondary)] uppercase tracking-wider px-5 py-3">Income</th>
                    <th className="text-right text-xs font-medium text-[var(--text-secondary)] uppercase tracking-wider px-5 py-3">Expenses</th>
                    <th className="text-right text-xs font-medium text-[var(--text-secondary)] uppercase tracking-wider px-5 py-3">Net</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-[var(--border)]">
                  {data.map((row) => {
                    ...
                    return (
                      <tr key={row.month} className="hover:bg-[var(--bg-hover)] transition-colors">
                        <td className="px-5 py-4 text-sm font-medium text-[var(--text-primary)]">{row.month}</td>
