    ...
    <div className="p-6 bg-white min-h-full">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          <button onClick={() => setShowResponseForm(true)} className="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors">Record Response</button>
          <button onClick={() => setShowSurveyForm(true)} className="flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 transition-colors">
      ...
      {!loading && (
        <div className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row gap-6">
            <div className="flex flex-col items-center justify-center border border-gray-200 rounded-xl p-6 min-w-[220px]">
              <NpsGauge score={npsScore} />
              <p className="text-xs text-gray-400 mt-2">{total} total response{total !== 1 ? "s" : ""}</p>
            </div>
            <div className="flex-1 grid grid-cols-3 gap-4">
              <div className="border border-gray-200 rounded-xl p-5">
                <div className="flex items-center gap-2 mb-1"><span className="w-3 h-3 rounded-full bg-green-400" /><span className="text-xs font-medium text-gray-500">Promoters (9-10)</span></div>
                <p className="text-2xl font-bold text-green-600">{promoters.length}</p>
                <p className="text-xs text-gray-400">{pctOf(promoters.length)}%</p>
              </div>
              <div className="border border-gray-200 rounded-xl p-5">
                <div className="flex items-center gap-2 mb-1"><span className="w-3 h-3 rounded-full bg-yellow-400" /><span className="text-xs font-medium text-gray-500">Passives (7-8)</span></div>
                <p className="text-2xl font-bold text-yellow-600">{passives.length}</p>
                <p className="text-xs text-gray-400">{pctOf(passives.length)}%</p>
              </div>
              <div className="border border-gray-200 rounded-xl p-5">
                <div className="flex items-center gap-2 mb-1"><span className="w-3 h-3 rounded-full bg-red-400" /><span className="text-xs font-medium text-gray-500">Detractors (0-6)</span></div>
                <p className="text-2xl font-bold text-red-600">{detractors.length}</p>
                <p className="text-xs text-gray-400">{pctOf(detractors.length)}%</p>
              </div>
            </div>
          </div>
                ...
                {trendPoints.map((tp, i) => {
                  ...
                  return (
                    <div key={i} className="flex-1 flex flex-col items-center gap-1">
                      <span className="text-[10px] font-medium text-gray-600">{tp.score}</span>
                      ...
                      <span className="text-[10px] text-gray-400">{tp.label}</span>
                    </div>
                  );
                })}
              </div>
            ...
            <div className="border border-gray-200 rounded-xl p-5">
              ...
              <div className="space-y-2">
                {Object.entries(clientScores).map(([cid, data]) => {
                  ...
                  return (
                    <div key={cid} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                      <span className="text-sm text-gray-700">{data.name}</span>
                      <div className="flex items-center gap-3">
                        <span className="text-xs text-gray-400">{data.scores.length} response{data.scores.length !== 1 ? "s" : ""}</span>
            ...
            <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50" onClick={() => setShowSurveyForm(false)}>
              <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-xl" onClick={e => e.stopPropagation()}>
                ...
                <div className="space-y-3">
                  ...
                  <input value={emailSubject} onChange={e => setEmailSubject(e.target.value)} placeholder="Email subject" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                </div>
                <div className="flex gap-2 mt-4 justify-end">
                  <button onClick={() => setShowSurveyForm(false)} className="px-4 py-2 text-sm text-gray-600 hover:text-gray-800">Cancel</button>
                  <button onClick={createSurvey} className="px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700">Create</button>
                </div>
              </div>
            </div>
          )}
            ...
            <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50" onClick={() => setShowResponseForm(false)}>
              <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-xl" onClick={e => e.stopPropagation()}>
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    <label className="text-xs font-medium text-gray-500 mb-1 block">Score (0-10)</label>
                    <div className="flex gap-1">
                      {Array.from({ length: 11 }, (_, i) => (
                        <button key={i} onClick={() => setRespScore(i)}
                          className={`flex-1 py-2 text-xs font-medium rounded transition-colors ${respScore === i ? (i >= 9 ? "bg-green-500 text-white" : i >= 7 ? "bg-yellow-500 text-white" : "bg-red-500 text-white") : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>{i}</button>
                      ))}
                    </div>
                  </div>
                  <textarea value={respComment} onChange={e => setRespComment(e.target.value)} placeholder="Comment (optional)" rows={3} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                </div>
                <div className="flex gap-2 mt-4 justify-end">
                  <button onClick={() => setShowResponseForm(false)} className="px-4 py-2 text-sm text-gray-600 hover:text-gray-800">Cancel</button>
                  <button onClick={recordResponse} disabled={!respClientId} className="px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 disabled:opacity-50">Record</button>
                </div>
              </div>
            </div>
          )}
