      ...
      <div className="p-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <h1 className="text-2xl font-bold text-[var(--text-primary)]">Workflow Recipes</h1>
            ...
            <button
              ...
              className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors ${
                activeCategory === cat
                  ...
                  : "text-[var(--text-secondary)] border-transparent hover:text-[var(--text-secondary)]"
              }`}
            >
              {cat}
            </button>
        ...
        {loading ? (
          <div className="flex items-center justify-center py-20">
            <div className="text-sm text-[var(--text-tertiary)]">Loading recipes...</div>
          </div>
        ) : filtered.length === 0 ? (
          <div className="bg-[var(--bg-base)] border border-[var(--border)] rounded-xl p-12 text-center">
            <p className="text-sm text-[var(--text-secondary)]">No recipes in this category.</p>
          </div>
        ) : (
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  ...
                  <span
                    className={`text-xs font-medium px-2 py-0.5 rounded-full ${
                      CATEGORY_COLORS[recipe.category] ?? "bg-[var(--bg-surface)] text-[var(--text-secondary)]"
                    }`}
                  >
                    {recipe.category}
                  </span>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="px-3 py-1.5 text-xs font-medium text-[var(--text-secondary)] bg-[var(--bg-base)] rounded-lg hover:bg-[var(--bg-hover)] transition-colors"
                  >
                    Details
                  </button>
                  <button
                    ...
                    className="px-3 py-1.5 text-xs font-medium text-[var(--primary-foreground)] bg-[var(--accent)] rounded-lg hover:bg-[var(--accent-hover)] disabled:opacity-50 transition-colors"
                  >
